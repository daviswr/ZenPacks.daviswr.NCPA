""" Monitors per-processor NCPA metrics """
...
class Processor(PythonDataSourcePlugin):
    """ NCPA per-processor data source plugin """
    ...
    def collect(self, config):
        ...
        cores = output.get('percent', [list(), ''])[0]
        ...
        for datasource in config.datasources:
            ...
            stats = dict()
            # CPUs are ID'd 0 to n number of cores
            # Effectively, the list index
            if len(cores) > int(datasource.component):
                stats['percent'] = cores[int(datasource.component)]
